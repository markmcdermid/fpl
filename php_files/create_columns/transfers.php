...
$key2 = 'transfers_id';
$transferKeyArray = array("player_id"=>"INT", "web_name"=>"TEXT", "date"=>"TEXT", "now_cost" => "INT", "selected_by_percent" => "DECIMAL(11,1)","transfers_out"=>"INT", "transfers_in"=>"INT", "transfers_out_event"=>"INT", "transfers_in_event"=>"INT");
foreach ($transferKeyArray as $key => $value) {
	$sqlAddCols = "ALTER TABLE `player_transfers` ADD `$key` $value NOT NULL AFTER `$key2`";
	$sqlAddCol = "ALTER TABLE `player_transfers` ADD `now_cost` INT NOT NULL AFTER `date`";
	$sqlDROP = "ALTER TABLE `player_transfers` DROP `$key`";
